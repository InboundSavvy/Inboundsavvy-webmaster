...
| An article lists itself under "Next in the series" | Platform bug, not a missing feature: the exclusion is implemented but `Astro.params.entry` comes back null, so it never fires - see `workflow-automation#97` for the open candidates | File it against the layout/routing (not the schema); scaffold from primitives only until that lands (see 8h) |
